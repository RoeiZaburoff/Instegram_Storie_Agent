...
| `CHROME_USER_DATA_DIR` | Persistent Chrome profile directory that stores the Instagram login session. Defaults to `./playwright-profile-pixel7`. |
...
| `MOBILE_DEVICE` | Playwright device descriptor used for mobile mode. Defaults to `Pixel 7`. |
...
| `DEBUG_STEP_MODE` | Reserved flag for step-by-step debugging. Defaults to `false`. |
...
3. Optional: verify or record the same Pixel 7 mobile flow with Playwright codegen:

   ```bash
   npx playwright codegen \
     --browser=chromium \
     --device="Pixel 7" \
     --user-data-dir="$(pwd)/playwright-profile-pixel7" \
     https://www.instagram.com/
   ```

4. Start the agent with a visible browser and the same fixed Pixel 7 profile folder:

   ```bash
   HEADLESS=false \
   CHROME_USER_DATA_DIR="$(pwd)/playwright-profile-pixel7" \
   MOBILE_EMULATION=true \
   MOBILE_DEVICE="Pixel 7" \
   npm start
   ```

5. Send a test webhook request, for example with `DRY_RUN=false` and a valid local media file:
...
6. Log in to Instagram in the opened browser and approve any 2FA/security checks.
7. Rerun the same webhook request after login. The session should be reused from `playwright-profile-pixel7`.
...
```bash
HEADLESS=false CHROME_USER_DATA_DIR="$(pwd)/playwright-profile-pixel7" MOBILE_EMULATION=true MOBILE_DEVICE="Pixel 7" npm start
```
