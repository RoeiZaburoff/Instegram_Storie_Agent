...
export const config = {
  ...
  storiesDir: process.env.STORIES_DIR ?? DEFAULT_STORIES_DIR,
  screenshotDir: process.env.SCREENSHOT_DIR ?? path.resolve('screenshots'),
  downloadDir: process.env.DOWNLOAD_DIR ?? path.resolve('tmp'),
  whatsappUploadDir: process.env.WHATSAPP_UPLOAD_DIR ?? path.resolve('tmp', 'whatsapp'),
  instagramUrl: process.env.INSTAGRAM_URL ?? 'https://www.instagram.com/',
  chromeUserDataDir: path.resolve(process.env.CHROME_USER_DATA_DIR ?? DEFAULT_CHROME_USER_DATA_DIR),
